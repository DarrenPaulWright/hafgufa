...
export default class Legend extends Control {
	constructor(settings = {}) {
		...
		self[BACKGROUND] = self.elementD3().append('rect')
	...
	items: method.array({
		set(newValue) {
			...
			if (newValue.length) {
				...
				self[TITLE] = self.elementD3().selectAll('.legend-title')
				...
				self.elementD3().selectAll('g').remove();

				self[ITEM_ELEMENTS] = self.elementD3().selectAll('g')
					.data(self[ITEMS])
					.enter()
					.append('g')
